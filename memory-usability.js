...
(() => {
  ...
  const report={version:VERSION,total:0};
  ...
  function install(q,n){
    const t=tier(q),len=[4,5,6][t],base=12+mod(n*7+Number(q.constructVariant||1)*5,55),vals=[];
    for(let i=0;i<len;i++)vals.push(10+mod(base+i*11+n*3,80));
    const seq=shuffle([...new Set(vals)],n*97+Number(q.constructVariant||1)*13);
    while(seq.length<len){const x=10+mod(base+seq.length*17+n,80);if(!seq.includes(x))seq.push(x);}
