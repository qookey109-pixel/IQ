...
(() => {
  ...
  function update(q,n){const t=tier(q),v=variant(q),start=20+mod(n*7+v*3,20),count=[2,3,4][t],patterns=[[1,2],[-1,1],[1,3],[-1,2],[1,1],[-1,3],[1,4],[-1,1]];let cur=start;const ops=[];for(let i=0;i<count;i++){const [sign,val]=patterns[mod(v+i+n,patterns.length)];ops.push([sign,val]);cur+=sign*val;}q.stim=`起始 ${start}；${ops.map(([s,val])=>`${s>0?'增加':'減少'} ${val}`).join('；')}`;q.q='照剛才的順序逐步更新，最後是多少？';q.e=`依序更新後得到 ${cur}。`;installOptions(q,String(cur),[cur-2,cur-1,cur+1,cur+2].filter(x=>x>=10&&x<100).map(String),'low-arithmetic-memory-update');}
