...
(() => {
  ...
  function generateCandidate({history=[],random=Math.random,pool=bank}={}){
    ...
    for(let attempt=0;attempt<160;attempt++){
      ...
      for(const domain of DOMAINS){
        const families=shuffle([...new Set(pool.filter(q=>q.d===domain).map(q=>q.taskFamily))],random);
        ...
        for(let assignTry=0;assignTry<80&&!assigned;assignTry++){
          ...
          for(let i=0;i<families.length;i++){
            const candidates=pool.filter(q=>q.d===domain&&q.taskFamily===families[i]&&q.difficulty===tiers[i]);
  ...
  window.IQ_DIVERSITY={...(window.IQ_DIVERSITY||{}),selectForm,validateForm,productionFormSize:42,familyDistinctPerDomain:true};
  window.IQ_BANK_META={...(window.IQ_BANK_META||{}),totalItems:2058,selectedItems:42,surfaceVariantsPerConstruct:7,bankTopology:'294x7=2058',formBlueprint:'6 domains x 7 families; 2 easy + 3 medium + 2 hard'};
  window.IQ_42_FORM={version:'F42-2026.09.1',items:42,domains:6,itemsPerDomain:7,familiesPerDomain:7,difficulty:{easy:2,medium:3,hard:2},targets};
})();