...
// Stable calibration anchors are reserved from scored forms so the linking block never drifts.
(() => {
  'use strict';
  ...
  function validateForm(form){
    ...
    if(xs.length!==42)errors.push('form must contain 42 items');
    const ids=new Set(xs.map(q=>q.id));if(ids.size!==42)errors.push('form must contain 42 unique item IDs');
    if(xs.some(q=>reservedCalibrationAnchorIds.has(q.id)))errors.push('formal form must not contain reserved calibration anchors');
    for(const domain of DOMAINS){
