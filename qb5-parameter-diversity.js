...
(() => {
  ...
  // Memory concrete stimuli use 32 genuine surface variants per construct block.
  save('memory-position');register('memory-position',(q,c)=>{old['memory-position'](q,c);const len=4+c.t,base=1000+c.n*20,vals=Array.from({length:len},(_,i)=>base+i*3),pos=mod(memorySerial(c.n)+c.v,len);q.stim=vals.join('　');q.q=`剛才序列中，第 ${pos+1} 個項目是什麼？`;q.e=`第 ${pos+1} 個項目是 ${vals[pos]}。`;set(q,String(vals[pos]),vals.filter((_,i)=>i!==pos).slice(0,3).map(String));q.surfaceVariant=memorySerial(c.n)+1;});
  save('memory-update');register('memory-update',(q,c)=>{const s=memorySerial(c.n),start=20+s,count=2+c.t;let cur=start;const ops=[[true,c.v+1]];cur+=c.v+1;for(let i=1;i<count;i++){const val=1+mod(s+c.v*2+i*3,8),plus=mod(i+c.v,2)===0;ops.push([plus,val]);cur+=plus?val:-val;}q.type='memory';q.stim=`起始 ${start}；${ops.map(([p,x])=>`${p?'增加':'減少'} ${x}`).join('；')}`;q.q='依剛才出現的順序逐步套用更新，最後數量是多少？';q.e=`依序計算後得到 ${cur}。`;setNum(q,cur);q.surfaceVariant=s+1;});
  save('memory-reorder');register('memory-reorder',(q,c)=>{old['memory-reorder'](q,c);const len=4+c.t,vals=Array.from({length:len},(_,i)=>500+c.n*11+i*5);q.stim=vals.join('　');let out;if(c.v===0)out=[...vals.slice(1),vals[0]];else if(c.v===1)out=[vals.at(-1),...vals.slice(0,-1)];else if(c.v===2)out=[...vals].reverse();else if(c.v===3)out=[vals[1],vals[0],...vals.slice(2)];else if(c.v===4){out=[...vals];const j=out.length-2;[out[1],out[j]]=[out[j],out[1]];}else if(c.v===5)out=[...vals.slice(2),...vals.slice(0,2)];else if(c.v===6)out=[vals[0],...vals.slice(1).reverse()];else out=[...vals.slice(-2),...vals.slice(0,-2)];const correct=out.join(' → ');set(q,correct,[[...out].reverse().join(' → '),vals.join(' → '),[...out.slice(1),out[0]].join(' → ')]);q.e=`依指定重排規則，得到 ${correct}。`;q.surfaceVariant=memorySerial(c.n)+1;});
