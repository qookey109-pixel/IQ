...
(() => {
  ...
  const enrichedBase = base.map((q) => {
    ...
    return {
      ...q,
      id: q.id || `qb2-${DOMAIN_SLUG[q.d] || "item"}-${String(index + 1).padStart(2, "0")}`,
  ...
  function readPreviousForm() {
    try {
      const parsed = JSON.parse(localStorage.getItem(LAST_FORM_KEY) || "[]");
  ...
  function pickTier(pool, difficulty, count) {
    ...
    const fresh = shuffle(tier.filter(q => !previousForm.has(q.id)));
    const repeats = shuffle(tier.filter(q => previousForm.has(q.id)));
  ...
  function pickForDomain(domain) {
    ...
    if (picked.length < 5) {
      ...
      const freshRemainder = shuffle(pool.filter(q => !used.has(q.id) && !previousForm.has(q.id)));
      const repeatRemainder = shuffle(pool.filter(q => !used.has(q.id) && previousForm.has(q.id)));
      picked.push(...[...freshRemainder, ...repeatRemainder].slice(0, 5 - picked.length));
    }
  ...
  try {
    localStorage.setItem(LAST_FORM_KEY, JSON.stringify(selectedIds));
  } catch {
    // Storage can be unavailable in private/restricted modes; selection still works.
  }
