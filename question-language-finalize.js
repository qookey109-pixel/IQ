...
(() => {
  ...
  const scaleVisual=(v,a,b,s,subject)=>{
    if(v===0||v===5){
      ...
      return svg(`<text x="180" y="22" text-anchor="middle" font-size="13">${subject}</text><text x="68" y="48" text-anchor="middle" font-size="12">原圖</text><line x1="28" y1="84" x2="108" y2="84" stroke="currentColor" stroke-width="5"/><text x="68" y="111" text-anchor="middle" font-size="15">原長 ${original}</text><line x1="135" y1="84" x2="218" y2="84" stroke="currentColor" stroke-width="2"/><polygon points="218,84 207,78 207,90" fill="currentColor"/><text x="176" y="68" text-anchor="middle" font-size="15">× ${s}</text><text x="286" y="48" text-anchor="middle" font-size="12">放大後</text><line x1="236" y1="84" x2="336" y2="84" stroke="currentColor" stroke-width="5"/><text x="286" y="111" text-anchor="middle" font-size="15">長 ${scaled}</text>`,'比例縮放線段圖');
    }
    const target=v===1?'長 ?':v===2?'寬 ?':v===3?'周長 ?':v===4||v===7?'面積 ?':'長＋寬 ?';
    const action=v===6?'平移':`× ${s}`;
    return svg(`<text x="180" y="20" text-anchor="middle" font-size="13">${subject}</text><text x="82" y="42" text-anchor="middle" font-size="12">原圖</text><rect x="34" y="55" width="96" height="72" fill="none" stroke="currentColor" stroke-width="3"/><text x="82" y="150" text-anchor="middle" font-size="14">長 ${a}</text><text x="18" y="91" text-anchor="middle" font-size="14" transform="rotate(-90 18 91)">寬 ${b}</text><line x1="148" y1="91" x2="214" y2="91" stroke="currentColor" stroke-width="2"/><polygon points="214,91 203,85 203,97" fill="currentColor"/><text x="181" y="74" text-anchor="middle" font-size="15">${action}</text><text x="284" y="42" text-anchor="middle" font-size="12">${v===6?'平移後':'放大後'}</text><rect x="233" y="50" width="102" height="82" fill="none" stroke="currentColor" stroke-width="3"/><text x="284" y="157" text-anchor="middle" font-size="15">${target}</text>`,'比例縮放矩形圖');
  };
  ...
  for(const q of bank){
    ...
    if(q.taskFamily==='scale-drawing'){
      ...
      const line=`線段 ${scaleLineLabels[s]}`,obj=scaleRectObjects[s];
      let subject=v===0||v===5?line:obj;
      if(v===0){q.q=`${line} 原長 ${a} 格，放大 ${factor} 倍後長多少格？`;q.e=`${a} × ${factor} = ${correctOf(q)}。`;}
      else if(v===1){q.q=`${obj}的原圖長 ${a}、寬 ${b}，等比例放大 ${factor} 倍。放大後的長是多少？`;q.e=`長度同樣放大 ${factor} 倍：${a} × ${factor} = ${correctOf(q)}。`;}
      else if(v===2){q.q=`${obj}的原圖長 ${a}、寬 ${b}，等比例放大 ${factor} 倍。放大後的寬是多少？`;q.e=`寬度同樣放大 ${factor} 倍：${b} × ${factor} = ${correctOf(q)}。`;}
      else if(v===3){q.q=`${obj}的原圖長 ${a}、寬 ${b}，等比例放大 ${factor} 倍。放大後周長是多少？`;q.e=`原周長是 2 × (${a} + ${b})，再乘 ${factor}，得到 ${correctOf(q)}。`;}
      else if(v===4){q.q=`${obj}的原圖長 ${a}、寬 ${b}，等比例放大 ${factor} 倍。放大後面積是多少？`;q.e=`面積會乘上倍率的平方：${a} × ${b} × ${factor}² = ${correctOf(q)}。`;}
      else if(v===5){q.q=`${line} 放大 ${factor} 倍後長 ${a*factor} 格。原長是多少格？`;q.e=`用放大後長度除以倍率：${a*factor} ÷ ${factor} = ${correctOf(q)}。`;}
      else if(v===6){factor=1;q.q=`${obj}長 ${a}、寬 ${b}。只做平移、不縮放。平移後長與寬的和是多少？`;q.e=`平移不改變尺寸，所以 ${a} + ${b} = ${correctOf(q)}。`;}
      else {q.q=`${obj}的原圖長 ${a}、寬 ${b}，長和寬都放大成 ${factor} 倍。放大後面積是多少？`;q.e=`新面積為 (${a} × ${factor}) × (${b} × ${factor}) = ${correctOf(q)}。`;}
      q.visual=scaleVisual(v,a,b,factor,subject);
