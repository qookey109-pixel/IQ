...
(() => {
  ...
  function gridDisplacement(q,n,v,t){
    ...
    install(q,`(${ex}, ${ey})`,[`(${ex+1}, ${ey})`,`(${ex-1}, ${ey})`,`(${ex}, ${ey+1})`,`(${ex}, ${ey-1})`,`(${ey}, ${ex})`],'spatial-grid-visual-path',{kind:'grid-displacement',start:[sx,sy],end:[ex,ey],moves},svg(z,'座標路徑圖'),'grid-displacement');
  }
  ...
  function viewpoint(q,n,v,t){const s18=mod(n,18),start=mod(s18,8),a=1+Math.floor(s18/8),b=1+mod(Math.floor(s18/4)+v,2);let delta,desc;
    ...
    else if(v===2){delta=a+1-b;desc=`先順時針 ${a+1} 格，再逆時針 ${b} 格`;}
    else if(v===3){delta=2*a;desc=`順時針轉 ${a} 個四分之一圈`;}
    else if(v===4){delta=4+(a-1);desc=`先轉向正後方，再順時針 ${a-1} 格`;}
    else if(v===5){delta=-2*a;desc=`逆時針轉 ${a} 個四分之一圈`;}
    else if(v===6){delta=a+2-b;desc=`先順時針 ${a+3} 格，再逆時針 ${b+1} 格`;}
    else {delta=-(a+2)+b;desc=`先逆時針 ${a+2} 格，再順時針 ${b} 格`;}
