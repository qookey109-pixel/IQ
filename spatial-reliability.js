...
(() => {
  ...
  const svg=(inner,label='空間推理圖')=>`<svg class="qb5-spatial-svg" viewBox="0 0 360 220" width="360" height="220" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${esc(label)}" xmlns="http://www.w3.org/2000/svg">${inner}</svg>`;
  ...
  function gridDisplacement(q,n,v,t){
    const N=9+t,sx=1+mod(n*2+v,3),sy=1+mod(n*3+v,3),a=1+mod(n+v,3),b=1+mod(n*2+v,2);
    ...
    let z='';for(let k=0;k<N;k++){const X=ox+k*cell,Y=oy-k*cell;z+=`<line x1="${X}" y1="${oy}" x2="${X}" y2="${oy-(N-1)*cell}" stroke="currentColor" stroke-opacity=".14"/><line x1="${ox}" y1="${Y}" x2="${ox+(N-1)*cell}" y2="${Y}" stroke="currentColor" stroke-opacity=".14"/>`;if(k%2===0){z+=`<text x="${X}" y="207" text-anchor="middle" font-size="10">${k}</text><text x="55" y="${Y+3}" text-anchor="end" font-size="10">${k}</text>`;}}
    z+=`<text x="${ox+(N-1)*cell+16}" y="${oy+4}" font-size="11">x</text><text x="${ox-4}" y="${oy-(N-1)*cell-10}" font-size="11">y</text>`;
    for(let i=0;i<pts.length-1;i++){const [x1,y1]=pts[i],[x2,y2]=pts[i+1];z+=arrow(ox+x1*cell,oy-y1*cell,ox+x2*cell,oy-y2*cell);}
    z+=`<circle cx="${ox+sx*cell}" cy="${oy-sy*cell}" r="6" fill="currentColor"/><text x="${ox+sx*cell-10}" y="${oy-sy*cell+20}" font-size="11">S</text><circle cx="${ox+ex*cell}" cy="${oy-ey*cell}" r="7" fill="none" stroke="currentColor" stroke-width="3"/><text x="${ox+ex*cell+9}" y="${oy-ey*cell+4}" font-size="11">E</text>`;
    q.q='依右側座標圖的箭頭從 S 移動到 E。E 的座標是？';q.e=`沿圖中路徑讀取終點，E 位於 (${ex}, ${ey})。`;
    install(q,`(${ex}, ${ey})`,[`(${ex+1}, ${ey})`,`(${ex}, ${ey-1})`,`(${ey}, ${ex})`],'spatial-grid-visual-path',{kind:'grid-displacement',start:[sx,sy],end:[ex,ey],moves},svg(z,'座標路徑圖'),'grid-displacement');
  }

  const DIRS=['北','東北','東','東南','南','西南','西','西北'];
  function compassSvg(start,steps){const cx=180,cy=108,r=70,ang=i=>(i*45-90)*Math.PI/180;let z=`<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="currentColor" stroke-opacity=".28"/>`;
    DIRS.forEach((d,i)=>{const a=ang(i),x=cx+Math.cos(a)*92,y=cy+Math.sin(a)*92+4;z+=`<text x="${x}" y="${y}" text-anchor="middle" font-size="11">${d}</text>`;});
    const a=ang(start),x=cx+Math.cos(a)*58,y=cy+Math.sin(a)*58;z+=arrow(cx,cy,x,y);z+=`<text x="180" y="205" text-anchor="middle" font-size="12">${esc(steps)}</text>`;return svg(z,'羅盤心像旋轉圖');}
  function viewpoint(q,n,v,t){const start=mod(n*3+v*2,8),a=1+mod(n+v,3),b=1+mod(n*2+v,2);let delta,desc;
    if(v===0){delta=a;desc=`順時針轉 ${a} 格（每格 45°）`;}
    else if(v===1){delta=-a;desc=`逆時針轉 ${a} 格（每格 45°）`;}
    else if(v===2){delta=2-b;desc=`先順時針 2 格，再逆時針 ${b} 格`;}
    else if(v===3){delta=2*a;desc=`順時針轉 ${a} 個四分之一圈`;}
    else if(v===4){delta=4;desc='轉向正後方';}
    else if(v===5){delta=-2*a;desc=`逆時針轉 ${a} 個四分之一圈`;}
    else if(v===6){delta=3-b;desc=`先順時針 3 格，再逆時針 ${b} 格`;}
    else {delta=-3+b;desc=`先逆時針 3 格，再順時針 ${b} 格`;}
    const end=mod(start+delta,8),correct=DIRS[end],wrong=[DIRS[mod(end+1,8)],DIRS[mod(end-1,8)],DIRS[mod(end+4,8)]];
    q.q=`觀察羅盤。箭頭目前指向「${DIRS[start]}」，依圖示轉向後最後指向哪個方向？`;q.e=`從 ${DIRS[start]} 依序做轉向，最後指向 ${correct}。`;
  ...
  function heightMap(n,v,t){const R=t===0?2:3,C=t===2?4:3;const g=[];for(let r=0;r<R;r++){const row=[];for(let c=0;c<C;c++)row.push(1+mod(n*5+v*7+r*3+c*5+r*c,4+t));g.push(row);}if(new Set(g.flat()).size===1)g[0][0]=Math.max(1,g[0][0]-1);return g;}
  function stackSvg(g){const R=g.length,C=g[0].length,cell=Math.min(42,150/Math.max(R,C)),ox=180-C*cell/2,oy=100-R*cell/2;let z='';for(let r=0;r<R;r++)for(let c=0;c<C;c++){const x=ox+c*cell,y=oy+r*cell;z+=`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="none" stroke="currentColor"/><text x="${x+cell/2}" y="${y+cell/2+5}" text-anchor="middle" font-size="16">${g[r][c]}</text>`;}z+=`<text x="180" y="${oy+R*cell+22}" text-anchor="middle" font-size="11">前方 ↑ 看入圖中</text><text x="180" y="204" text-anchor="middle" font-size="11">數字＝該位置堆疊高度</text>`;return svg(z,'方塊柱投影高度圖');}
  function stackHidden(q,n,v,t){const g=heightMap(n,v,t),front=v%2===0;let skyline;
    ...
    const correct=skyline.join('–');const alt1=[...skyline].reverse().join('–');const alt2=skyline.map((x,i)=>i===0?Math.max(1,x-1):x).join('–');const alt3=skyline.map((x,i)=>i===skyline.length-1?x+1:x).join('–');
    q.q=front?'依俯視高度圖，從圖下方「前方」朝上看，輪廓由左到右的高度是哪一列？':'依俯視高度圖，從右側觀看，輪廓由前到後的高度是哪一列？';q.e=`每個視線方向取可遮蔽後的最高柱，得到 ${correct}。`;
    install(q,correct,[alt1,alt2,alt3],'spatial-stack-projection',{kind:'stack-projection',grid:g,view:front?'front':'right',skyline},stackSvg(g),'stack-projection');
  }
  ...
  function cutGrid(n,v,t){const R=4+t,C=5+t,g=Array.from({length:R},()=>Array(C).fill(1));const k=1+(v>=3?1:0)+(v>=6?1:0);for(let i=0;i<k;i++){const side=mod(v+i,4),pos=mod(n*3+v*2+i*2,side<2?C:R);let r,c;if(side===0){r=0;c=pos;}else if(side===1){r=R-1;c=pos;}else if(side===2){r=pos;c=0;}else{r=pos;c=C-1;}g[r][c]=0;}return g;}
  function cutSvg(g){const R=g.length,C=g[0].length,cell=Math.floor(145/Math.max(R,C)),ox=180-C*cell/2,oy=96-R*cell/2;let z='';for(let r=0;r<R;r++)for(let c=0;c<C;c++){const x=ox+c*cell,y=oy+r*cell;z+=`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="${g[r][c]?'none':'currentColor'}" fill-opacity="${g[r][c]?'0':'.18'}" stroke="currentColor" stroke-opacity=".55"/>`;}z+=`<text x="180" y="196" text-anchor="middle" font-size="11">深色格＝已裁除；每格邊長 1</text>`;return svg(z,'單位方格裁切圖');}
  ...
  function scaleSvg2(w,h,sx,sy,tx,ty){const cell=18,ox=100,oy=170;let z='';for(let i=0;i<=10;i++){z+=`<line x1="${ox+i*cell}" y1="${oy}" x2="${ox+i*cell}" y2="${oy-8*cell}" stroke="currentColor" stroke-opacity=".12"/><line x1="${ox}" y1="${oy-i*cell}" x2="${ox+10*cell}" y2="${oy-i*cell}" stroke="currentColor" stroke-opacity=".12"/>`;}
    z+=`<rect x="${ox}" y="${oy-h*cell}" width="${w*cell}" height="${h*cell}" fill="none" stroke="currentColor" stroke-width="3"/><text x="${ox+w*cell/2}" y="${oy+18}" text-anchor="middle" font-size="11">寬 ${w}</text><text x="${ox-12}" y="${oy-h*cell/2}" text-anchor="middle" font-size="11" transform="rotate(-90 ${ox-12} ${oy-h*cell/2})">高 ${h}</text><text x="180" y="28" text-anchor="middle" font-size="11">水平×${sx} · 垂直×${sy}${tx||ty?` · 再平移 (${tx}, ${ty})`:''}</text>`;return svg(z,'座標縮放圖');}
  function scaleDrawing(q,n,v,t){const w=2+mod(n+v,3),h=2+mod(n*2+v,3),sx=v<3?2:1+mod(v,2),sy=v<3?sx:2+mod(v,2),tx=v>=6?1+mod(n,2):0,ty=v>=6?1+mod(n+v,2):0;const ex=w*sx+tx,ey=h*sy+ty;
    q.q='依右側圖示，矩形左下角固定在原點；完成標示的縮放與平移後，右上角座標是多少？';q.e=`原右上角為 (${w}, ${h})；完成變換後為 (${ex}, ${ey})。`;
    install(q,`(${ex}, ${ey})`,[`(${ex+1}, ${ey})`,`(${ex}, ${ey+1})`,`(${ey}, ${ex})`],'spatial-scale-coordinate-transform',{kind:'scale-drawing',w,h,sx,sy,tx,ty,end:[ex,ey]},scaleSvg2(w,h,sx,sy,tx,ty),'scale-drawing');
  }
  ...
  function axesSvg(x,y,mode,c){const sx=a=>180+a*18,sy=a=>110-a*18;let z='<line x1="25" y1="110" x2="335" y2="110" stroke="currentColor" stroke-opacity=".2"/><line x1="180" y1="18" x2="180" y2="202" stroke="currentColor" stroke-opacity=".2"/>';
    ...
    if(mode==='xc')z+=`<line x1="${sx(c)}" y1="18" x2="${sx(c)}" y2="202" stroke="currentColor" stroke-width="2" stroke-dasharray="6 5"/><text x="${sx(c)+5}" y="30" font-size="10">x=${c}</text>`;
    if(mode==='yc')z+=`<line x1="25" y1="${sy(c)}" x2="335" y2="${sy(c)}" stroke="currentColor" stroke-width="2" stroke-dasharray="6 5"/><text x="30" y="${sy(c)-5}" font-size="10">y=${c}</text>`;
    z+=`<circle cx="${sx(x)}" cy="${sy(y)}" r="6" fill="currentColor"/><text x="${sx(x)+8}" y="${sy(y)-8}" font-size="11">P(${x},${y})</text>`;return svg(z,'座標鏡射圖');}
  function mirrorCoordinate(q,n,v,t){let x=2+mod(n+v,4),y=1+mod(n*2+v,4),c=1+mod(n,2),a,mode,rule;
    ...
    install(q,`(${a[0]}, ${a[1]})`,[`(${-a[0]}, ${a[1]})`,`(${a[0]}, ${-a[1]})`,`(${a[1]}, ${a[0]})`],'spatial-mirror-diagram-grounded',{kind:'mirror-coordinate',point:[x,y],rule,answer:a,mode,c},axesSvg(x,y,mode,c),'mirror-coordinate');
  }
  ...
  function pathGridSvg(N,S,T,B){const cell=Math.floor(165/N),ox=98,oy=20;let z='';for(let r=0;r<N;r++)for(let c=0;c<N;c++){const k=`${r},${c}`,x=ox+c*cell,y=oy+r*cell,b=B.has(k);z+=`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="${b?'currentColor':'none'}" fill-opacity="${b?'.22':'0'}" stroke="currentColor" stroke-opacity=".22"/>`;}const p=a=>[ox+a[1]*cell+cell/2,oy+a[0]*cell+cell/2],[sx,sy]=p(S),[tx,ty]=p(T);z+=`<circle cx="${sx}" cy="${sy}" r="6" fill="currentColor"/><text x="${sx+8}" y="${sy-8}" font-size="11">S</text><circle cx="${tx}" cy="${ty}" r="7" fill="none" stroke="currentColor" stroke-width="3"/><text x="${tx+8}" y="${ty-8}" font-size="11">E</text><text x="180" y="210" text-anchor="middle" font-size="11">只能上下左右；深色格不可通行</text>`;return svg(z,'受阻最短路徑圖');}
  function shortestPath(q,n,v,t){const N=8+t, row=N-2, S=[row,1],T=[row,N-2],wall=2+mod(n+v,N-4),gap=mod(Math.floor(n/(N-4))+v,N-3),B=new Set();for(let r=0;r<N;r++)if(r!==gap)B.add(`${r},${wall}`);B.delete(S.join(','));B.delete(T.join(','));const c=bfs(N,S,T,B),man=Math.abs(S[0]-T[0])+Math.abs(S[1]-T[1]);if(c==null||c<=man)throw new Error(`spatial reliability: obstacle failed to force detour ${q.id}`);
