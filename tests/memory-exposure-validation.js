...
assert.strictEqual(memoryItems.length, 1008, 'expected 1008 QB4 working-memory items');

const expected = { easy: 4, medium: 5, hard: 6 };
const counts = { easy: 0, medium: 0, hard: 0 };
for (const q of memoryItems) {
  counts[q.difficulty] += 1;
  assert.strictEqual(policy.getSeconds(q), expected[q.difficulty], `${q.id}: memory exposure must follow difficulty policy`);
  assert.strictEqual(q.type, 'memory', `${q.id}: working-memory item type`);
  assert.ok(q.stim, `${q.id}: stimulus required`);
}

assert.deepStrictEqual(counts, { easy: 406, medium: 406, hard: 196 });
...
console.log('QB4 adaptive memory exposure validation PASS');
console.log('1008 memory items use 4s easy / 5s medium / 6s hard, one exposure, untimed response.');
