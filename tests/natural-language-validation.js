...
assert.ok(averages.filter(q=>Number(q.constructVariant)===3).every(q=>/再加入 \d+ 後/.test(q.q)),'add-observation average items must explicitly say the value is added');
...
assert.match(css,/max-height:\s*min\(29dvh,\s*280px\)/,'desktop spatial diagrams must remain readable without dominating the question card');
