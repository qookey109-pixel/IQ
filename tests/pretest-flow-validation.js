...
for (const source of [scoring, assessment]) {
  assert.ok(!/ageYears|ageBand|IQ_PARTICIPANT_PROFILE/.test(source), 'age must not participate in current CPI scoring');
}
...
console.log('18–65 whole-year age -> 4 unscored practices -> unchanged 42-item formal assessment; age excluded from CPI scoring.');
