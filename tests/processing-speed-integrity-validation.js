...
const bank=window.IQ_QUESTION_BANK,speed=bank.filter(q=>q.d==='處理速度');assert.strictEqual(bank.length,5908);assert.strictEqual(speed.length,1008);assert.strictEqual(window.IQ_PROCESSING_SPEED_INTEGRITY?.version,'PSI-2026.09.1');assert.strictEqual(window.IQ_PROCESSING_SPEED_INTEGRITY?.total,1008);assert.strictEqual(window.IQ_BANK_META?.processingSpeedIntegrity,'PSI-2026.09.1');
...
const positions=[0,0,0,0];for(const q of bank)positions[q.a]++;assert.deepStrictEqual(positions,[1477,1477,1477,1477]);const sig=q=>JSON.stringify([q.q,q.stim||'',q.cells||[],q.visual||'',[...q.o].map(String).sort()]);assert.strictEqual(new Set(bank.map(sig)).size,5908,'final signatures remain unique');assert.strictEqual(window.IQ_OPTION_QUALITY_REPORT?.cueRiskItems,0,'final option audit clean');
