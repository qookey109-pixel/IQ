...
assert.ok(html.includes('styles.bundle.css?v=20260919-rc1'),'RC CSS cache key must be current');
assert.ok(html.includes('runtime.bundle.js?v=20260919-rc1'),'RC runtime cache key must be current');
...
for(const marker of [
  ...
  'Cognitive Performance Index (CPI) 0–100',
...
assert.ok(readme.includes('RC-2026.09.19-1'));
...
console.log('RC-2026.09.19-1: cache-busted local bundles, 42-item/CPI-only authority, governance locks, and fallback boundaries are consistent.');
