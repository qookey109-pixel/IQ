...
// Memory span must increase by tier for families designed around span/load.
...
// Ordering constraints: harder tiers use more elements.
const order=bank.filter(q=>q.taskFamily==='ordering-constraints');
function orderSize(q){const m=q.q.match(/^([A-Z、]+) 必須排/);return m?m[1].split('、').length:0;}
...
// Spatial grid task grows its search space by tier; every spatial item has SVG.
...
// Exact-match scanning length scales 4/5/6 characters by tier.
...
// Matrix prompts must require induction; never reveal the rule in the stem.
...
console.log('294 templates; construct-linked tiers; increasing memory/search load; 1,008 spatial SVG items.');