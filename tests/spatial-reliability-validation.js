...
for(const q of spatial){
  ...
  if(q.taskFamily==='grid-displacement'){
    let [x,y]=d.start;for(const [dx,dy] of d.moves){x+=dx;y+=dy;}assert.deepStrictEqual([x,y],d.end,`${q.id}: path endpoint`);assert.strictEqual(q.correctContent,`(${x}, ${y})`,`${q.id}: endpoint answer`);assert.ok(!/向右|向左|向上|向下/.test(q.q),`${q.id}: moves belong in diagram, not prose`);
    ...
    const end=[d.w*d.sx+d.tx,d.h*d.sy+d.ty];assert.deepStrictEqual(end,d.end,`${q.id}: scale transform`);assert.strictEqual(q.correctContent,`(${end[0]}, ${end[1]})`,`${q.id}: scale answer`);
  }else if(q.taskFamily==='mirror-coordinate'){
    const [x,y]=d.point,c=d.c;let a;if(v===1)a=[-x,y];else if(v===2)a=[x,-y];else if(v===3)a=[-x,-y];else if(v===4)a=[y,x];else if(v===5)a=[-y,-x];else if(v===6)a=[2*c-x,y];else if(v===7)a=[-x,2*c-y];else a=[-y,-x];assert.deepStrictEqual(a,d.answer,`${q.id}: mirror transform`);assert.strictEqual(q.correctContent,`(${a[0]}, ${a[1]})`,`${q.id}: mirror answer`);
