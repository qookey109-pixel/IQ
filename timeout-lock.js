...
const timingBaseFinishTest = finishTest;
finishTest = function () {
  // Timing owns deadline settlement and total-duration bookkeeping only.
  // Result scoring/rendering stays delegated to app.js so every fallback remains CPI-only.
  saveElapsedBeforeLeave();
  settleTimedQuestions();
  stopTimer();
  stopTotalTimer();
  return timingBaseFinishTest();
};
